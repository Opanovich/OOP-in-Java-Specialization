...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		ellipse(x, y, 10, 10);
		y += 25;
		quad(x - radius, y - radius, x + radius, y - radius,
		...
		x = 75;
		y = 100;
		text("City Marker", x, y);
		text("Land Quake", x, y += 25);
		text("Ocean Quake", x, y += 25);
		text("Size ~ Magnitude", x - 25, y += 25);
		text("Shallow", x, y += 30);
		text("Intermediate", x, y += 25);
		text("Deep", x, y += 25);
		text("Past Day", x, y += 25);
	}
	...
	private void printQuakes() 
	{
		...
		// map for all unique countries with quakes
		Map<String, Integer> landQuakes = new HashMap<>();
		List<String> countries = new ArrayList<>();
		for (Marker quakeMarker: quakeMarkers) {
			if (((EarthquakeMarker) quakeMarker).isOnLand()) {
				String country = quakeMarker.getStringProperty("country");
				if (landQuakes.containsKey(country)) {
					landQuakes.replace(country, landQuakes.get(country) + 1);
				} else {
		...
		// sorting and printing country and number of quakes
		Collections.sort(countries);
		for (String country : countries) {
			System.out.println(country + " has " + landQuakes.get(country) + " quake(s).");
		}
		System.out.println("There are " + numOceanQuakes + " quake(s) in the ocean.");
